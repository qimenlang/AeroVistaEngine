#!/usr/bin/env bash
# ============================================================
# One-click seam-grid multichannel launch (git-bash / MSYS2).
#   - 1 Host  : aerovistaViewHost.exe  (MFC GUI, reads viewhost.json next to exe)
#   - 3 IG    : vsgEngine.exe with scene_seam_grid_ig_main/left/right.json
# IG consoles are hidden via PowerShell; logs go to logs/ig_seam_grid_<name>.{out,err}.log
#
# Same Host / IG ports as run_multichannel.sh — do not run both at once.
#
# SINGLE SOURCE OF TRUTH: run_seam_grid.bat is only a thin
# Windows wrapper that forwards here. Edit exe paths / IG list /
# log layout ONLY in this file, never in the .bat.
#
# Usage:
#   scripts/run_seam_grid.sh            # start
#   scripts/run_seam_grid.sh stop       # stop all
#   scripts\run_seam_grid.bat           # equivalent (forwards here)
# ============================================================
set -euo pipefail
...
for cfg in scene_seam_grid_ig_main scene_seam_grid_ig_left scene_seam_grid_ig_right; do
...
for name in main left right; do
    cfg="$CFG_DIR/scene_seam_grid_ig_$name.json"
    outlog="$LOG_DIR/ig_seam_grid_$name.out.log"
    errlog="$LOG_DIR/ig_seam_grid_$name.err.log"
...
echo
echo "Launched 1 Host + 3 seam-grid IG (no consoles; logs under $LOG_DIR)."
echo "To stop: scripts/run_seam_grid.sh stop"
