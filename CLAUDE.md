...
每次更新 `doc/` 下任一文档（新增/删除/改名/改章节标题/改文件名），都要检查「是否有其他文档引用了它」，有则一并同步：

- **索引文件**：`doc/private/README.md` 的文件清单须与实际文件一致。
- **交叉引用链接**：private 各文档相互引用，路径随移动/改名而变化，须同步。
- **目录锚点/章节编号**：§ 标题或编号变了，指向它的锚点、其它文档里「详见 §X」要同步。
- **移动/改名**：所有指向旧路径的链接都要改。

核查方式：更新后 `rg` 该文档的文件名/标题/编号，确认所有引用处一致；触及则更新，未触及则跳过。

### 决策日志只读规则

「决策日志」是**用户亲手维护**的文档，AI 只读、不修改、不追加、不演进。

- **识别方式（按文件名精确匹配，不按内容）**：文件名含「决策日志」四个字，或位于 `**/决策日志/` 目录下。不按「内容是否含决策」判断——设计文档里大量出现「决策」「否决」字样，按内容会误伤本应由 AI 演进的设计文档。
- **规则**：AI 产出的方案与决策写入设计文档（`doc/design/`），由用户审核；用户审核后**亲手摘记**到决策日志；AI 不得在任何情况下改写、追加、删除决策日志内容，哪怕用户要求「同步/更新」。
