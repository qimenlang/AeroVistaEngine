...
| -------------------- | ------------------------------------------------------ | ------ | ----------------------------------------- |
| Cognitive complexity | clang-tidy `readability-function-cognitive-complexity` | **20** | `engine/**` TUs（Tests **exempt**）；一等库 `thirdparty/sync`、`thirdparty/config` |
| Cyclomatic CCN       | lizard (`scripts/lizard_engine.py`, `-C 15`)           | **15** | `engine/source/**/*.cpp`；`thirdparty/sync/src`；`thirdparty/config/src` |
...
## 7. 审查问题确认（review → 登记 → 确认 → 改）

Authority: `.cursor/rules/review-problem-confirmation.mdc`。

对设计文档 / 代码做**审查**时，发现的问题**不得直接修改正文**——先登记、报告、讨论确认，再处理。

### 核心

「审查请求」（如「检查一下是否还有漏洞」）**只产出问题清单，不产出改动**。审查 ≠ 授权修改。

### 流程（写死）

1. **登记**：把所有审查发现的问题列到设计文档的「问题自查」节（表格：`#` / 问题 / 关联章节 / 状态）。
2. **报告**：向用户报告问题清单，并按性质分类——**客观矛盾 / 事实错误**（文档内部自相矛盾、与代码不符）vs **设计决策 / 缺口**（需用户拍板）。
3. **讨论**：逐条与用户讨论确定方案；**用户是决策方，AI 不擅自定方案**（尤其设计决策类）。
4. **处理**：方案确认后才修改正文；已解决的从「问题自查」节删除或标记「已解决」。
