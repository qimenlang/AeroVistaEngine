...
主线程（帧循环，SynchronSystem::preFrame）:
  ...
  6. `IgSync::update()` 帧级维护（外推冻结 / RUNNING 状态）；随后 `Engine::applyLastHostEye` 写相机
```
...
| `CIGI-sof-echo` | SOF 回显 | 每条 IGCtrl 回一条 SOF（须先 `drainIncoming` 再读计数） | `[integration][sync][status][sof][cigi][CIGI-sof-echo]` |
...
| 多通道齐集 | **扩展位** | 命令面跑通后 |
| Host 等齐**所有** IG 再发下一包 | **扩展位** | 全通道 lockstep（模块设计 §9 P1）；**不是** 单 master SofGated，也**不是** Present Barrier（三套集合点见 [帧同步设计.md](./帧同步设计.md) §1.1） |
...
| 删 `SyncPaceConfig`/`SendPace`/`FrameGate` 节拍机制 | **已实现（2026-08 删除）**：旧 `frameGate` API 已删。缺省仍 FreeRun；后续节拍用 `hostConfig.messageSync`（SofGated = 单 master，见 [帧同步设计.md](./帧同步设计.md) §4），**不**把已删的 FrameGate 原样加回。等齐所有 IG 再发仍属 §11 扩展，≠ Present Barrier |
