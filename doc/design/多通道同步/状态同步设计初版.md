...
> > **定制扩展（2026-08 / 2026-09）**：`OnPacketReceived` 为虚函数，默认实现 = dynamic_cast + 多播投递。需业务翻译/过滤的报文统一在**订阅回调**内完成（`Engine::onEntityPositionCtrl` 按 `EntityID` 分流眼点/命令实体即为范例）；未定制报文直接用模板实例化，零样板。
...
| 眼点链路收敛 | **已实现（2026-08 翻译收敛回 processor；2026-09 翻译移至业务侧 + 多播 + 通用捕获 + 同步只 LLA）**：`EntityPositionCtrlV4` 走 UDP 通用捕获 `PacketCaptureProc<CigiEntityPositionCtrlV4>`，ownship（`EntityID==0`）眼点**原始报文**经多播投递（原 `EyeCaptureProc` 定制类已删，2026-09）；`HostEyePose` 翻译（恒 LLA 字段提取）在业务侧回调（`Engine::onEntityPositionCtrl`），经 `SynchronSystem::queueHostEyePose` 入队 `_pendingHostEye`；`update()` 帧决策三态化（未连接保末帧 / 新输入校验应用 / 无新输入 stale），offset 合成（`applyComposed`）仍在 `update()` 路径。同步只 LLA（2026-09）：`HostEyePose` 无 frame 判别、`tryAcceptPendingEye`/`HostEyeCoordFrame` 已删 |
| 命令实体位姿（TCP 命令面摆放） | **已实现（2026-08 / 2026-09 多播 + 通用捕获收敛 + 同步只 LLA）**：`IgSync` 对 `EntityPositionCtrlV4` 注册 TCP 通用捕获 `_entityPoseProc`（命令面一次性摆放；与 UDP 眼点同 PacketID，双链路均向同一回调多播，原 `EntityPoseControlProc` 定制类已删）；`Engine::onEntityPositionCtrl` 按 `EntityID` 分流——眼点翻译入队决策器、命令实体恒 `Detach`+LLA → `updateEntityPose(id, lla, ypr)` 写 entityMap 语义位姿 + `recomputeEntityTransform`；`SyncCommandTests` `[entity-pose]` E2E 覆盖（TCP 摆放，Ellipsoid） |
