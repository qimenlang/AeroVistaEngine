...
> - **缓存数据结构：CCL 报文值拷贝（写死，2026-08）**：CCL `IncomingHandlerTbl` 是**复用单例**，报文对象每次解包被覆盖，不能直接缓存**指针**——processor 必须**值拷贝**。眼点链路（2026-08 翻译收敛回 processor；2026-09 起 `EntityPositionCtrlV4` 走通用捕获投递原始报文、`HostEyePose` 翻译移至业务侧回调）：其余报文以 CCL 原生类型经订阅投递，后续按需翻译为自建结构。
...
> > - `addCallback<PacketT>(callback)` 追加业务回调，报文捕获时同步多播投递；回调在主线程解包时调用，**只做轻量翻译/入队/置标志**（§8.1 重量约束），重量业务留帧循环消费队列；生命周期同业务 processor（宿主保证覆盖会话）。订阅调用时会确保会话已创建。
...
> > 业务侧选型：**事件/响应类报文**（Los/Position/Sensor/EventNotification 等一次性）用订阅免轮询；**决策类**（眼点/IGCtrl/时钟）维持既有数据流（`SynchronSystem::preFrame` 收包 → `CameraDriver::update` 决策+应用；2026-09 决策上移 Engine 后抽出 CameraDriver）。
...
| 眼点链路收敛 | **已实现（2026-08 翻译收敛回 processor；2026-09 翻译移至业务侧 + 多播 + 通用捕获 + 同步只 LLA；2026-09 决策上移 Engine 后抽出 `CameraDriver`）**：`EntityPositionCtrlV4` 走 UDP 通用捕获 `PacketCaptureProc<CigiEntityPositionCtrlV4>`，ownship（`EntityID==0`）眼点**原始报文**经多播投递（原 `EyeCaptureProc` 定制类已删，2026-09）；`HostEyePose` 翻译（恒 LLA 字段提取）在 `CameraDriver::onOwnshipEyePose`，经 `CameraDriver::queueHostEyePose` 入队 `_pendingHostEye`；`CameraDriver::update()` 帧决策三态化（未连接保末帧 / 新输入校验应用 / 无新输入 stale），offset 合成（`CameraDriver::compose` / `applyComposed`）在 `update` 路径。同步只 LLA（2026-09）：`HostEyePose` 无 frame 判别、`tryAcceptPendingEye`/`HostEyeCoordFrame` 已删 |
