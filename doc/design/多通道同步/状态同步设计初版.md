...
> - **IG 侧**：数据面 + 命令面**按链路分会话**解包（2026-08 双 session）——UDP 队列喂 `_udpSession`（数据面 IGCtrl/眼点 + 命令面 UDP 报文）、TCP 队列喂 `_tcpSession`（命令面 TCP 报文）。**UDP 收包已拆到独立 I/O 线程**（`udpLoop`：非阻塞 `recv` → 过滤握手面 → 入队 `_udpPayloadQueue`，并在 recv 时刻记录 `receivedAtUs`）；主线程收包入口收敛为 **`IgSync::drainIncoming()`**（统一 drain TCP+UDP → 按链路喂各 session 解包，见 §8.1 收包入口对等化；空队列最多再等 2ms（两个 UDP I/O 轮询周期）后再试，覆盖刚到内核尚未入队的数据报及调度抖动），`IgSync::update()` 只做帧级维护（冻结 + 状态）。sync 库内部注册基础设施 processor（`IgCtrlCaptureProc` 捕获帧号/时间戳、`SofCaptureProc` 计数、`EntityPositionCtrlV4` 走 UDP/TCP 双链路通用捕获 `PacketCaptureProc<CigiEntityPositionCtrlV4>`——ownship 眼点与命令实体同 PacketID，业务回调按 `EntityID` 分流，见 §8.1 通用模式）；业务 processor 经 `registerEventProcessor` 注册。旧的 `TimeStampValid` 分流与 `dispatchCommandPlaneFrame` 已删除。
...
> | **业务 processor** | engine 层定义，经 `registerEventProcessor` 注册 | 文本指令分发等；命令实体位姿已由 sync 库双链路通用捕获投递 → engine `addCallback<CigiEntityPositionCtrlV4>` 回调按 `EntityID` 分流写 entityMap（§12） | 依赖 engine 场景状态（entityMap） | engine 注册 |
>
...
> 已实现并全绿（2026-08）：命令面 + 数据面（矛盾 A）+ **双向命令面**均完成——`SyncCommandTests.cpp` 全绿（30 条顶层用例：线格式契约、TCP/UDP 收发对等 E2E、分帧、双 session 隔离、IGCtrl 自动填充、帧头去重、all-packets 订阅、命令实体位姿），`HostIGTests.cpp` 数据面帧节拍/SOF/断线全绿 + **首包约束负向用例**（IG 收包首包非 IGCtrl / Host 收包首包非 SOF 被 CCL 拒绝）全绿（`flushTcp/flushUdp` 真发送 + `registerEventProcessor` 真注册 + 消息级分帧 + IG 统一 CCL 会话解包 + `HostSync::update` 已删、业务侧组装 + IG 发送接口 + Host 收包 processor + 收包入口对等化）。验收断言落在**可观察结果**（processor 分发副作用、文本内容、分帧边界、帧节拍计数、CCL 首包拒绝）。
>
