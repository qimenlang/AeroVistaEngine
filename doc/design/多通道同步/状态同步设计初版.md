...
> **坐标系判据（写死，2026-09）**：位姿语义（LLA vs 本地 XYZ）由**运行时场景判据**（有无 `EllipsoidModel`）决定，**不再由 `AttachState` 决定**；`coordFrame` 仅在「场景无椭球」时驱动注入。`AttachState` 仅为 CIGI 线格式字段语义（`Detach` 时 LLA 字段有效、`Attach` 时 XYZ 字段有效），是场景判据派生的伴随值，不承载业务坐标系选择。命令实体见 [实体与运动控制设计.md](./实体与运动控制设计.md) §4.2，眼点见 [lla位姿传输设计.md](./lla位姿传输设计.md) §5。
...
| 眼点链路收敛 | **已实现（2026-08 翻译收敛回 processor；2026-09 翻译移至业务侧 + 多播 + 通用捕获）**：`EntityPositionCtrlV4` 走 UDP 通用捕获 `PacketCaptureProc<CigiEntityPositionCtrlV4>`，ownship（`EntityID==0`）眼点**原始报文**经多播投递（原 `EyeCaptureProc` 定制类已删，2026-09）；`HostEyePose` 翻译（AttachState→frame + 字段提取）在业务侧回调（`Engine::onEntityPositionCtrl`），经 `SynchronSystem::queueHostEyePose` 入队 `_pendingHostEye`；`update()` 帧决策三态化（未连接保末帧 / 新输入校验应用 / 无新输入 stale），frame 校验（`tryAcceptPendingEye`）与 offset 合成（`applyComposed`）仍在 `update()` 路径。`HostEyePose`/`HostEyeCoordFrame` 归 `SyncConfig.h` |
| 命令实体位姿（TCP 命令面摆放） | **已实现（2026-08 / 2026-09 多播 + 通用捕获收敛）**：`IgSync` 对 `EntityPositionCtrlV4` 注册 TCP 通用捕获 `_entityPoseProc`（命令面一次性摆放；与 UDP 眼点同 PacketID，双链路均向同一回调多播，原 `EntityPoseControlProc` 定制类已删）；`Engine::onEntityPositionCtrl` 按 `EntityID` 分流——眼点翻译入队决策器、命令实体 Attach/Detach → `updateEntityPose(id, pos, ypr, frame)` 写 entityMap 语义位姿 + `recomputeEntityTransform`；`SyncCommandTests` `[entity-pose]` E2E 覆盖（TCP 摆放） |
