...
> 已实现并全绿（2026-08）：命令面 + 数据面（矛盾 A）+ **双向命令面**均完成——`SyncCommandTests.cpp` 全绿（30 条顶层用例：线格式契约、TCP/UDP 收发对等 E2E、分帧、双 session 隔离、IGCtrl 自动填充、帧头去重、all-packets 订阅、命令实体位姿），`HostIGTests.cpp` 数据面帧节拍/SOF/断线全绿 + **首包约束负向用例**（IG 收包首包非 IGCtrl / Host 收包首包非 SOF 被 CCL 拒绝）全绿（`flushTcp/flushUdp` 真发送 + `addCallback` 真注册 + 消息级分帧 + IG 统一 CCL 会话解包 + `HostSync::update` 已删、业务侧组装 + IG 发送接口 + Host 收包 processor + 收包入口对等化）。验收断言落在**可观察结果**（processor 分发副作用、文本内容、分帧边界、帧节拍计数、CCL 首包拒绝）。
>
...
| IG→Host 上报（双向命令面） | IG 经 `igSync().outMsgWithSofTcp()<<CigiIGMsgV4→flushTcp()` 发消息 → Host `addCallback` + `drainIncoming()` 解包 → processor 收到（MsgID + 文本） | `[acceptance][bdd][e2e]` | `IG sends a message to Host and Host processor receives it`（`SyncCommandTests.cpp`） | 已写（绿） |
...
| processor 归属 EventProcess + 订阅投递 | `IgSync::addCallback<CigiCollDetVolDefV4>()` / `HostSync::addCallback<CigiCollDetVolRespV4>()` 捕获时同步投递（原 `takeReceivedCollDetVolDef/Resp`、`takeReceived<...>` 专用/通用拉接口已删，2026-08） | `[integration]` | Vol 对断言（§8.1 通用模式） | 已写（绿） |
| IG→Host UDP 上报 | IG 经 `igSync().outMsgWithSofUdp()<<CigiIGMsgV4→flushUdp()` 发消息 → Host `addCallback` + `drainIncoming()` 解包 → processor 收到（MsgID + 文本） | `[acceptance][bdd][e2e]` | `IG sends a UDP message and Host processor receives it`（`SyncCommandTests.cpp`） | 已写（绿） |
