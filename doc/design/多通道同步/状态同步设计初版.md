...
| 多通道齐集 | **扩展位** | 命令面跑通后 |
| **实体预加载（EntityID → 模型映射表）** | **已设计**（见 [实体与运动控制设计.md](./实体与运动控制设计.md)）：IG 启动全量预建 + Switch 显隐 + 一次编译；配置作 `id → path` 映射。**② 装配已实现**（`initSceneFromEntities`：共享几何 / Switch / pose 缺省 0 / 失败跳过）；Host `EntityCtrlV4` 只切 Switch 仍待③ | `place` 等指令引用的实体来源；实体实例在 IG 启动预建（含编译），`EntityCtrlV4` 不再承担「加载/创建」，只切显隐 |
