...
3. CIGI 标准包**没有** Present ready/release；初步走 **UDP 同步面**上的 Extension Packet（与高频 IGCtrl 同面）。**不**改走 TCP（队头阻塞，见 [socket总结.md](../../notes/socket总结.md)）。丢包：同号连发降丢包率（§3.5）；仍可丢，跟下一帧或 §3.3 超时。**禁止**停等 ACK 再重发。
