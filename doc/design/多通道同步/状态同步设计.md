...
CCL 在 **initialize** 按模式设 `SetSynchronous`（FreeRun `false`，SofGated `true`）。非 master 的 SOF 仍计数/健康观察，**不**放行发帧。master 未接入或超时：不发下一数据面 IGCtrl；不得 silent 改回 FreeRun。`masterChannelId` 与 peer 表如何对齐（HELLO 是否带 `channelId`）实现时钉。
...
void flushUdp();                   // 打包 _udpSession 的 omsg → sendTo HostTarget.udpPortRecv（UDP）；空缓冲跳过；重置帧头去重
```
...
| `outMsgWithSofUdp()` / `flushUdp()` | `IgSync` | IG→Host UDP 发送（开消息 + 自动前置 SOF，帧头去重），`flushUdp` 发到 `HostTarget::udpPortRecv`；绑定 `_udpSession` |
...
| `hostConfig.messageSync` / SofGated | **未实现**（§3.1 / §3.2）；FreeRun **已实现**（`CIGI-freerun`） |
