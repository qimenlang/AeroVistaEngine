...
| 现状 | 缺口 |
|------|------|
| TCP 握手已通（HELLO/HELLO_ACK），连接保留在 peer 表 / `_tcp` | **已实现（命令面）**：命令帧改 CCL 标准报文 + 消息级分帧器（§4.2）；数据面「单一 I/O 线程」留待数据面重构 |

> **实现进度（2026-08）**：命令面已落地「I/O 线程（`commandLoop`）recv+分帧入队 → 主线程（`runPendingCommands`）`ProcessIncomingMsg` → processor」。数据面（矛盾 A）仍在 `IgSync::update` 内 recv+`unpackHostFrame` 解包，尚未拆到单一 I/O 线程；UDP 命令面报文由 `update` 内 `dispatchCommandPlaneFrame` 喂命令面 session。
...
>
> **CCL 会话懒初始化（写死，性能决策）**：`CigiSession` 构造/析构在 MSVC Debug 下约 **80ms/op**——根因是 `CigiIncomingMsg` 内含 `EventList[0x10000]` + `CallBackList[0x10000]`（共 131072 个 `std::list`），Debug 迭代器（`_ITERATOR_DEBUG_LEVEL=2`）下每个 list 构造/析构都 new/delete 一个 `_Container_proxy`。因此会话**按需懒创建**：Host 在首次 `tcpOutgoing()/udpOutgoing()` 时创建（`ensureSession`），IG 在首次 `registerEventProcessor` 时创建（`ensureCmdSession`）；纯数据面端点（走手写 `packHostFrame`/`unpackHostFrame`，不注册 processor）不构造任何 CCL 会话。未注册 processor 时 `dispatchCommandPlaneFrame`/`runPendingCommands` 直接跳过命令面解包。
>
...
> 命令面已实现并全绿（2026-08）：`SyncCommandTests.cpp` 12 条测试全部通过（2 线格式 + 6 E2E + 4 分帧单测；`flushTcp/flushUdp` 真发送 + `registerEventProcessor` 真注册 + 消息级分帧 + 主线程解包）。验收断言落在**可观察结果**（processor 分发副作用、文本内容、分帧边界）。数据面（矛盾 A：业务组装 IGCtrl/眼点、删 `HostSync::update`）尚未实现，见 §12。断线检测、SOF 回显在 `HostIGTests.cpp` 已覆盖，本表一并收录。
...
> **已移除的验收点（不单列，原因见下）**：
> - 引用式组装 + flush 生效 —— 已被上表 6 条 E2E 的 `tcpOutgoing()<<…<<flushTcp()` / `udpOutgoing()<<…<<flushUdp()` 调用 + processor 收到断言完整覆盖，无独立断言价值（冗余）。
> - 业务组装 IGCtrl/眼点（无 `HostSync::update`）—— 属数据面重构（矛盾 A），当前 `HostSync::update` 仍保留、尚未实现，见 §12。
...
> - 同报文双链路区分 —— §4.1 已写明「靠收包来源区分，而非 PacketID」，但当前 TCP/UDP 命令面共用同一 session、协议层不区分来源，属业务层语义、无对应实现（过度设计）。
...
| 引用式发送接口（`tcpOutgoing`/`udpOutgoing` + `flush`） | **已实现**（命令面；`flushTcp`/`flushUdp` 真发送到 ready peers；`HostSync::update` 仍保留——数据面重构留待后续） |
| 去业务回执（ACK/NACK）+ 主线程解包执行 + processor 分发 | **已实现**（命令面：`registerEventProcessor` + 主线程 `ProcessIncomingMsg`；旧 `sendCommand`/handler 保留过渡） |
| 去掉 LOADMODEL / MOVEMODEL 枚举与旧载荷 | **待实现**（当前 `Engine::loadModelFromPayload` / `moveModelFromPayload` / `moveEntityById` 仍在） |
| 测试用例 | **已重写**（`SyncCommandTests.cpp` 12 条全绿，见 §10；数据面 `HostIGTests` 仍用 `HostSync::update`，留待数据面重构） |
