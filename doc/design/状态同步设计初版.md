...
> | 类 | 定义方 | 例子 | 依赖 | 注册方式 |
> |---|---|---|---|---|
> | **基础设施 processor** | sync 库**内部**（不暴露给 engine），统一定义于 `EventProcess.h/.cpp` | SOF 回显计数、IGCtrl 帧号/时间戳捕获、眼点捕获（`IgCtrlCaptureProc`/`EyeCaptureProc`/`SofCaptureProc`）、碰撞检测体积收发（`CollDetVolDefProc`/`CollDetVolRespProc`） | 只依赖 sync 库自身状态（`_sofReceivedCount`、`_lastFrameCntr`） | sync 库自行注册 |
...
> - **Host**：`SofCaptureProc`（SOF 回显）**两个 session 都注册**（数据面 SOF 走 UDP，IG TCP 上报的消息头也是 SOF）；碰撞检测响应 `CollDetVolRespProc` 注册 `_tcpSession`（响应走 TCP）；业务 processor（IG→Host 上报，如 `CigiIGMsgV4`）**两个 session 都注册**（IG 可能经 TCP 或 UDP 发来）。
> - **IG**：数据面 `IgCtrlCaptureProc`/`EyeCaptureProc` 注册 `_udpSession`（数据面 IGCtrl/眼点只经 UDP 到达）；碰撞检测定义 `CollDetVolDefProc` 注册 `_tcpSession`（定义走 TCP）；业务 processor（Host→IG 命令，如 `EntityPositionCtrlV4`/`SymbolTextDefV4`）**两个 session 都注册**（命令可能经 TCP 一次性下发或 UDP 实时下发）。
...
> - 已实例：`EyeCaptureProc`（缓存 `CigiEntityPositionCtrlV4` 值，getter 返回）、`IgCtrlCaptureProc`（缓存 `CigiIGCtrlV4` 值，帧号/时间戳由 `processIncomingUdp` 消费）、`SofCaptureProc`（SOF 计数）、碰撞检测 processor（`CollDetVolDefProc`/`CollDetVolRespProc`，见本段「碰撞检测收发」）。
>
...
| processor 归属 EventProcess | **已实现（2026-08）**：所有 CIGI 报文 processor 统一定义于 `EventProcess.h/.cpp`；`IgCtrlCaptureProc`/`EyeCaptureProc`/`SofCaptureProc`/`CollDetVolDefProc`/`CollDetVolRespProc` 迁入；删旧空壳 `IGCtrl`/`SofProcessor`（§8.1） |
| 碰撞检测体积收发（Vol 对） | **已实现（2026-08）**：Host TCP 发 `CollDetVolDefV4` → IG 内置 processor 缓存 → IG 回 `CollDetVolRespV4` → Host 内置 processor 缓存；取缓存接口 `IgSync::takeReceivedCollDetVolDef()` / `HostSync::takeReceivedCollDetVolResp()`；红测已转绿（§8.1）。Seg 对（`CollDetSegDef/Resp`）链路由测试自定义 processor 覆盖，无内置缓存 |
