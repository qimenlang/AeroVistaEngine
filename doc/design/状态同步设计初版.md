...
分帧逻辑：
  1. 维护拼帧缓冲
  2. recv → 塞入缓冲
  3. 缓冲 >= 2？→ 读 PacketSize（按检测到的字节序）→ 知完整长度
  4. 缓冲 >= PacketSize？→ 取完整一条 → 交给 CigiIncomingMsg::ProcessIncomingMsg
  5. 剩余继续（粘包/拆包都覆盖）
```
...
| 现状 | 缺口 |
|------|------|
| TCP 握手已通（HELLO/HELLO_ACK），连接保留在 peer 表 / `_tcp` | 命令帧改用 CCL 标准报文 + 分帧器（重构现有手写 `packCommandMsg`） |
...
> **HostSync 无 `update` 方法（写死）**：系统帧节拍（IGCtrl / 眼点）与实时控制位姿**均由业务侧（Engine）组装**进 `CigiOutgoingMsg`，`flush` 是唯一发送入口。业务侧每帧发送 = `udpOutgoing() << IGCtrl << 眼点 << 实时位姿` → `flushUdp()`；一次性命令 = `tcpOutgoing() << 报文` → `flushTcp()`。sync 层退化为纯传输管道，报文组装职责上移至业务侧。
...
> 当前 `SyncCommandTests.cpp` 为 **ATDD 红测**（stub 阶段），仅覆盖 5 条验收点（线格式绿 + 3 个 E2E 红）；其余验收点在开发阶段（实现 flush/分帧/processor 后）补测。验收断言应落在**可观察结果**（`entityTransform(7)` 位姿值、`sofSentCount`、processor 分发副作用），而非内部实现细节。

| 场景 | 断言要点 | 标签 | 测试用例 | 状态 |
|------|----------|------|----------|------|
| TCP 摆放生效 | Host 经 TCP 发 `EntityPositionCtrlV4` → IG 主线程解包 → processor 写 entityMap 位姿（`entityTransform(7)` 位姿更新） | `[acceptance][bdd][e2e]` | `Host places an entity pose over TCP via tcpOutgoing/flushTcp` | 已写（红） |
| UDP 实时控制 | Host 经 UDP 每帧发 `EntityPositionCtrlV4` → IG 帧内解包实时更新位姿 | `[acceptance][bdd][e2e]` | `Host streams real-time entity pose over UDP via udpOutgoing/flushUdp` | 已写（红） |
| 文本指令生效 | Host 经 TCP 发 `SymbolTextDefV4`（`"place 7 ..."`）→ IG 解包 → 首 token 分发执行 | `[acceptance][bdd][e2e]` | `Host sends a text command over TCP via SymbolTextDefV4` | 已写（红） |
| 多指令分发 | 不同指令名（place / reset 等）→ IG 按首 token 分发到对应逻辑 | `[acceptance][bdd][e2e]` | `IG dispatches different text commands by first token` | 已写（红） |
| **引用式组装 + flush 生效** | 业务侧 `tcpOutgoing() << 报文 → flushTcp()` 后 IG 收到；`udpOutgoing() << → flushUdp()` 同理 | `[acceptance][bdd][e2e]` | 无（隐含在 E2E 的 flush 调用中，无独立断言） | 未写 |
| **批量多报文单消息** | 一次 `<<` 多个报文（如 `EntityPositionCtrlV4` + `SymbolTextDefV4`）→ `flush` → IG 按 PacketID 分别分发到对应 processor | `[integration]` | `Host sends multiple packets in one message and IG dispatches each by PacketID` | 已写（红） |
| **业务组装 IGCtrl/眼点（无 HostSync::update）** | 业务侧自行 `<<` IGCtrl + 眼点 → `flushUdp()` → IG 收到帧节拍（取代原 `HostSync::update` 直发） | `[acceptance][bdd][e2e]` | 无 | 未写 |
| **processor 链路（收字节→解包→processor→entityMap）** | 喂字节 → 主线程 `ProcessIncomingMsg` → processor 捕获字段并写 entityMap 位姿 | `[integration]` | 无（现 E2E 断言 processor 被调，未断言 entityMap 位姿值） | 未写 |
| **SOF 回显（每条 IGCtrl 回一条）** | IG 主线程解包每条 IGCtrl 后回一条 SOF（`FrameCntr` 一一对应，`sofSentCount == igCtrlReceivedCount`） | `[integration]` | 无 | 未写 |
| 线格式契约 | `EntityPositionCtrlV4`（定长 48B）/ `SymbolTextDefV4`（变长 Text）正确编解码 | `[unit][wire-contract]` | `CigiEntityPositionCtrlV4 packs to fixed 48B` / `CigiSymbolTextDefV4 packs variable-length Text` | 已写（绿） |
| TCP 分帧 | 粘包/拆包下完整切出 CIGI 报文 | `[unit][wire-contract]` | 无 | 未写 |
| 同报文双链路区分 | 同一 `EntityPositionCtrlV4` 在 UDP（实时）与 TCP（摆放）语义不同，IG 按来源正确分派 | `[integration]` | 无 | 未写 |
| **多 peer 扇出** | Host 一次 `flushTcp()`/`flushUdp()` → 所有 ready IG 都收到同一批报文（广播语义，逐 peer send） | `[acceptance][bdd][e2e]` | `Host fans out a command to multiple IGs via flushTcp` | 已写（红） |
| 断线检测 | IG 掉线 → Host I/O 线程 EOF / 超时判断连、摘 peer | `[acceptance][bdd][e2e]` | 无 | 未写 |

> **关于「未写」测试**：剩余 6 条「未写」验收点分两类：**待开发阶段补测的验收契约**（引用式 flush 独立断言、processor 链路 entityMap 位姿、TCP 分帧、同报文双链路区分），以及**依赖未设计功能暂不可测**（业务组装 IGCtrl/眼点、SOF 回显——旧 `HostSync::update`/数据面仍运行导致断言无法隔离为红；断线检测——数据面 `HostIGTests` 已覆盖）。均**不删除**，待实现后补齐。
...
| 命令帧改为 CCL 标准报文（`EntityPositionCtrlV4` 摆放 / `SymbolTextDefV4` 文本指令，CCL Pack） | **待实现**（当前为手写 `cigi_wire::packCommandMsg`，借用 IGMsg 头、小端，过渡态） |
| TCP 分帧器（按 CIGI V4 `PacketSize` 字节序检测切流，不假设大端） | **待实现**（当前 `CommandFrameAssembler` 按小端 IGMsg 头切） |
| TCP/UDP 各自独立 CCL 会话 | **已无需**（架构改为「I/O 线程只收字节 + 主线程解包」，CCL 单线程化，Host/IG 各一套 `Cigi*Session` 即可） |
| 引用式发送接口（`tcpOutgoing`/`udpOutgoing` + `flush`，HostSync 无 `update`） | **待实现**（当前 `sendCommand` 两段回执 + seq + 三指令枚举；`HostSync::update` + `packHostFrame` 删除） |
| 去业务回执（ACK/NACK）+ 主线程解包执行 + processor 分发 | **待实现**（当前读循环线程解包 + 回 RECEIVED/RESULT + 旧 handler；改为 engine 注册 processor + 主线程解包） |
| 去掉 LOADMODEL / MOVEMODEL 枚举与旧载荷 | **待实现**（当前 `Engine::loadModelFromPayload` / `moveModelFromPayload` / `moveEntityById` 仍在） |
| 测试用例 | **待重写**（§10；当前 `SyncCommandTests.cpp` 的 E2E 用旧 `setPlaceHandler`/`setCommandHandler`，改为 `registerEventProcessor` + engine 定义 processor） |
