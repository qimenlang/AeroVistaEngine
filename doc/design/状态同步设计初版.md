...
- **数据面眼点 vs 命令面位姿的区分（写死，矛盾 A 后演进）**：数据面眼点（ownship `EntityID=0`）与命令面实体位姿（业务 `EntityID≠0`）都可能是 `IGCtrl + EntityPositionCtrlV4` 结构，区分由 processor 完成：
  - sync 库内部基础设施 `EyeCaptureProc` **只捕获 `EntityID==0`** 的眼点（UDP 数据面）；
  - sync 库内部 `EntityPoseControlProc`（TCP 命令面）**只投递 `EntityID≠0`** 的命令实体（过滤 `EntityID==0`）；engine 侧 `subscribe<CigiEntityPositionCtrlV4>` 订阅后按 `EntityID==0` 再保险过滤。
...
> | **业务 processor** | engine 层定义，经 `registerEventProcessor` 注册 | 文本指令分发等；命令实体位姿已由 sync 库 `EntityPoseControlProc`（基础设施，TCP 注册、过滤 `EntityID==0`）投递 → engine `subscribe<CigiEntityPositionCtrlV4>` 回调写 entityMap（§12） | 依赖 engine 场景状态（entityMap） | engine 注册 |
>
...
>
> **不再提供 `setPlaceHandler`/`setCommandHandler`（写死）**：业务逻辑收敛在 processor 的 `OnPacketReceived` / 订阅回调里（命令实体位姿：engine `subscribe<CigiEntityPositionCtrlV4>` 回调直接写 `entityMap` 位姿），无需再包一层 handler 间接。
>
...
| 命令实体位姿（TCP 命令面摆放） | **已实现（2026-08）**：`IgSync` 新增 `EntityPoseControlProc _entityPoseProc`（命令面一次性摆放，注册于 TCP；与 ownship 眼点同 PacketID，双 processor 均触发，`EntityPoseControlProc` 过滤 `EntityID==0`，§4.1 / cigi梳理.md 链路矩阵）；`Engine::initSync` 订阅 `subscribe<CigiEntityPositionCtrlV4>`，回调翻译 Attach/Detach → `updateEntityPose(id, pos, ypr, frame)` 写 entityMap 语义位姿 + `recomputeEntityTransform`；`SyncCommandTests` `[entity-pose]` E2E 覆盖（TCP 摆放） |
