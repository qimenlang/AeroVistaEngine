...
> - **通用捕获模板（写死，2026-08）**：为覆盖全 9 类报文收包，`EventProcess.h` 提供 `PacketCaptureProc<PacketT>`（继承 `CigiBaseEventProcessor` + `CaptureProcBase`）：`OnPacketReceived` 里 `dynamic_cast` + **整包值拷贝**（变长字段为 `std::string`/`vector`，深拷贝安全），带 `has()`/`take()`/`reset()`。`IgSync`/`HostSync` 各持一组按链路注册的模板实例。**数据交付双模式**：
> > - **拉**：`takeReceived<PacketT>()` 按类型取走（遍历注册表 + `dynamic_cast` 匹配，取走即清）；
> > - **推（订阅，2026-08）**：`subscribe<PacketT>(callback)` 注入业务回调，报文捕获时**同步值拷贝投递**（`setSink`）；与拉模式并存互不消费。回调在主线程解包时调用，**只做轻量翻译/入队/置标志**（§8.1 重量约束），重量业务留帧循环消费队列；生命周期同业务 processor（宿主保证覆盖会话）。订阅调用时会确保会话已创建。
> > 业务侧选型：**事件/响应类报文**（Los/Position/Sensor/EventNotification 等一次性）用订阅免轮询；**决策类**（眼点/IGCtrl/时钟）维持既有数据流（`preFrame → update → takePendingCameraPose`）；`takeReceived` 保留供测试与轮询场景。
> 全 9 类报文注册矩阵见 [cigi梳理.md](../notes/cigi梳理.md)「实现状态」。
> - 已实例：`EyeCaptureProc`（缓存 `CigiEntityPositionCtrlV4` 值，getter 返回）、`IgCtrlCaptureProc`（缓存 `CigiIGCtrlV4` 值，帧号/时间戳由 `processIncomingUdp` 消费）、`SofCaptureProc`（SOF 计数）、`PacketCaptureProc<...>`（其余全部可达报文，含 `CigiCollDetVolDefV4`/`CigiCollDetVolRespV4`——原 `CollDetVolDefProc`/`CollDetVolRespProc` 已收敛入模板，2026-08）。
>
...
| 双向碰撞检测体积应答（TCP） | Host 发 `CollDetVolDefV4` → IG 通用捕获缓存 → IG 回 `CollDetVolRespV4` → Host 通用捕获缓存 → `takeReceived<...>()` 取缓存验证（成对；首版默认值填充） | `[acceptance][bdd][e2e]` | `Host sends CollDetVolDef and IG replies CollDetVolResp over TCP`（`SyncCommandTests.cpp` §6，红测已转绿） | 已写（绿） |
| processor 归属 EventProcess + 通用捕获取缓存 | `IgSync::takeReceived<CigiCollDetVolDefV4>()` / `HostSync::takeReceived<CigiCollDetVolRespV4>()` 返回通用捕获缓存（原 `takeReceivedCollDetVolDef/Resp` 专用接口已删除，2026-08） | `[integration]` | Vol 对断言（§8.1 通用模式） | 已写（绿） |
...
| 通用捕获 + 订阅投递 | **已实现（2026-08）**：`PacketCaptureProc<PacketT>`（值拷贝 + has/take/reset）+ `IgSync`/`HostSync::subscribe<PacketT>(callback)`（捕获时同步投递，与 `takeReceived` 并存互不消费；回调轻量约束见 §8.1）；`SyncCommandTests` `[all-packets]` 覆盖 IgSync 与 HostSync 双侧订阅 |
...
| 碰撞检测体积收发（Vol 对） | **已实现（2026-08）**：Host TCP 发 `CollDetVolDefV4` → IG 通用捕获缓存 → IG 回 `CollDetVolRespV4` → Host 通用捕获缓存；取缓存经 `IgSync::takeReceived<CigiCollDetVolDefV4>()` / `HostSync::takeReceived<CigiCollDetVolRespV4>()`（原 `takeReceivedCollDetVolDef/Resp` 专用接口已删除，统一走模板）。Seg 对（`CollDetSegDef/Resp`）链路由测试自定义 processor 覆盖，无内置缓存 |
