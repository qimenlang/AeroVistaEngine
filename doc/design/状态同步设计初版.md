...
> - **通用捕获模板（写死，2026-08）**：为覆盖全 9 类报文收包，`EventProcess.h` 提供 `PacketCaptureProc<PacketT>`（继承 `CigiBaseEventProcessor` + `CaptureProcBase`）：`OnPacketReceived` 里 `dynamic_cast` + **整包值拷贝**（变长字段为 `std::string`/`vector`，深拷贝安全），带 `has()`/`take()`/`reset()`。`IgSync`/`HostSync` 各持一组按链路注册的模板实例，业务经 `takeReceived<PacketT>()` 按类型取走（遍历注册表 + `dynamic_cast` 匹配）。全 9 类报文注册矩阵见 [cigi梳理.md](../notes/cigi梳理.md)「实现状态」。
