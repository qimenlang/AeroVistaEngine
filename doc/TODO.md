...
- [ ] 现状：实体加载失败首版**不上报不处理**（实体静默缺失，IG 侧日志暴露）——`实体与运动控制设计.md` §7.1。
- [ ] 扩展：把加载失败的实体以**占位模型（如红立方体）**替换，避免画面缺失、提高可观测性（能直观看到「哪个实体加载失败、在哪个位置」）。登记于 §11 扩展位。
- [ ] 注意：占位替换与「加载结果上报」（IG→Host 事件通知，§11 扩展位）是两个可独立引入的增强；若二者都做，占位是 IG 本地呈现、上报是让 Host 感知，互不替代。
